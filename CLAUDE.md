...
A post is a markdown or MDX file at `src/content/posts/YYYY-MM-DD-slug.{md,mdx}`. The schema lives in `src/content/config.ts` — required: `title`, `date`. Optional: `blurb`, `cover`, `tags`, `draft`.
...
Required env vars on Vercel: `WRITE_PASSWORD`, `ANTHROPIC_API_KEY`, `GITHUB_TOKEN` (with `repo` scope, fine-grained for adamtpang/pangaea.blog), optionally `GITHUB_OWNER`, `GITHUB_REPO`, `GITHUB_BRANCH` (defaults match this repo).
...
- Voice: terse, considered, eclectic. Whole Earth Catalog energy. Sive.rs brevity. 150–400 words is the target for a typical post.
...
- Don't introduce a CMS. The git repo IS the CMS — /write just makes commits to it.
